...
class InitialGameInteractorTests: XCTestCase {
    ...
    func testIfResetWorks() throws {
        ...
        for i in 0...3 {
            XCTAssertEqual(interactor.players.value[i].correctAnswers,0)
            XCTAssertEqual(interactor.players.value[i].incorrectAnswers,0)
        }
        
        wordWorker.needsCorrectPoint = true

        for i in 0...3 {
            interactor.playFor(userIndex: i, meaning: "")
            XCTAssertEqual(interactor.players.value[i].correctAnswers,1)
            XCTAssertEqual(interactor.players.value[i].incorrectAnswers,0)
            XCTAssertEqual(interactor.players.value[i].getNetScore(), 1)
        }
        ...
        for i in 0...3 {
            interactor.playFor(userIndex: i, meaning: "")
            XCTAssertEqual(interactor.players.value[i].correctAnswers,1)
            XCTAssertEqual(interactor.players.value[i].incorrectAnswers,1)
            XCTAssertEqual(interactor.players.value[i].getNetScore(), 0)
        }
